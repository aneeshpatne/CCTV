...
The camera runs with its hardware AE, AGC, and AWB engines disabled. Startup explicitly freezes manual exposure, restores the saved image profile, and refuses to run the software loops unless camera readback confirms the automatic hardware controls are off. The tuned startup profile is 1247 shutter lines, a conservative launchd gain ceiling of `34/16` (about 2.13x), warmer fixed white-balance baseline `122/55/54`, tone `lumaOffset +24` with contrast registers `[48, 48, 48, 10]`, at XGA/quality 12.
