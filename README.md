...
The optional external floodlight integration uses `POST /api/lights/floodlight` with JSON `{"action":"on"}` or `{"action":"off"}`. Set `CCTV_FLOODLIGHT_BASE_URL` to the relay server origin to enable it. A scene at or below `CCTV_FLOODLIGHT_DARK_THRESHOLD` (default 18%) for `CCTV_FLOODLIGHT_OBSERVATION_SECONDS` (default 10 seconds) turns the light on. Because the floodlight itself brightens the camera view, it stays latched and performs a brief ambient-light probe every `CCTV_FLOODLIGHT_PROBE_INTERVAL_SECONDS` (default 15 minutes), waiting `CCTV_FLOODLIGHT_PROBE_SETTLE_SECONDS` before deciding whether to restore it. Motion produces one sub-second double pulse, inverted when the light is already on, and always restores the policy state. `CCTV_FLOODLIGHT_BRIGHT_THRESHOLD`, `CCTV_FLOODLIGHT_HTTP_TIMEOUT_SECONDS`, and the other timing variables tune the behavior. The native and Python fallback pipelines share the same policy.
