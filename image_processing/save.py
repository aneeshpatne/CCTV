...
def draw_overlay(frame, motion_boxes, motion_flag):
    ...
    cv2.putText(
        frame, txt, (10, 26), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA
    )
    # ROI poly
    cv2.polylines(
        frame,
...
def reader():
    ...
    cap.open()
    if not cap.isOpened():
        raise RuntimeError("Could not open ESP HTTP MJPEG stream")
    # prime first frame to get size
    ok, f = cap.read()
    if not ok:
        raise RuntimeError("Failed initial read")
    try:
